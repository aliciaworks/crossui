...
interface CodeGenerator {
    val target: ExportTarget
    fun generate(document: ResolvedDocument, typeName: String): GeneratedSource
}
...
object CrossUiCompiler {
    fun generate(
        ...
        return targets.map { target ->
            ...
            target.generator().generate(resolved, typeName)
        }
    }
    ...
    fun write(
        ...
        targets: Set<ExportTarget> = ExportTarget.entries.toSet(),
        typeName: String = "CrossUiGenerated",
    ): List<Path> = generate(document, targets, typeName).map { generated ->
...
private object SwiftUiGenerator : CodeGenerator {
    ...
    override fun generate(document: ResolvedDocument, typeName: String): GeneratedSource {
        val body = swiftNode(document.document.root, 2)
    ...
    private fun swiftNode(node: Node, depth: Int): String {
        val i = "    ".repeat(depth)
        val child = { extra: Int ->
            node.children.joinToString("\n") { swiftNode(it, depth + extra) }
        }
        return when (val kind = node.kind) {
            ...
            is NodeKind.Input -> {
                ...
                val field = if (kind.secure || kind.inputType == InputType.Password) {
                    "SecureField(\"$prompt\", text: .constant(\"${kind.value.swift()}\"))"
                } else {
                    "TextField(\"$prompt\", text: .constant(\"${kind.value.swift()}\"))"
                }
                ...
                if (kind.mode == NavigationMode.Tab) {
                    "$i${"TabView {\n"}" +
                        node.children.joinToString("\n") { route ->
                            ...
                            swiftNode(route, depth + 1) +
                                ".tabItem { Text(\"${title.swift()}\") }"
                        } + "\n$i}"
                } else {
                    "$i${"NavigationStack {\n${selected?.let { swiftNode(it, depth + 1) }.orEmpty()}\n$i}"}"
                }
            }
            is NodeKind.Route ->
                "$i${"VStack {\n${child(1)}\n$i}"}.navigationTitle(\"${kind.title.swift()}\")" +
                    if (kind.respectSafeArea) "" else ".ignoresSafeArea()"
            is NodeKind.PlatformView ->
                throw IllegalArgumentException(
                    "Platform view '${kind.name}' needs an explicit SwiftUI escape hatch.",
                )
            is NodeKind.Toggle ->
                "$i${"Toggle(\"${kind.label.orEmpty().swift()}\", isOn: .constant(${kind.checked}))"}" +
                    ".onChange(of: ${kind.checked}) { _, value in dispatch(\"${kind.onChange.swift()}\", String(value)) }" +
                    enabled(node) + accessibility(node)
            is NodeKind.Image ->
            ...
            is NodeKind.Slider ->
                "$i${"Slider(value: .constant(${kind.value}), in: ${kind.min}...${kind.max}, step: ${kind.step ?: 1.0})"}" +
                    ".onChange(of: ${kind.value}) { _, value in dispatch(\"${kind.onChange.swift()}\", String(value)) }" +
                    enabled(node) + accessibility(node)
            is NodeKind.Picker ->
                "$i${"Picker(\"\", selection: .constant(\"${kind.selected.swift()}\")) {\n"}" +
                    kind.options.joinToString("\n") {
                        "${"    ".repeat(depth + 1)}Text(\"${it.label.swift()}\").tag(\"${it.value.swift()}\")"
                    } + "\n$i}.onChange(of: \"${kind.selected.swift()}\") { _, value in dispatch(\"${kind.onChange.swift()}\", value) }"
            ...
            is NodeKind.Checkbox ->
                "$i${"Button { dispatch(\"${kind.onChange.swift()}\", String(!${kind.checked})) } label: { Label(\"${kind.label.orEmpty().swift()}\", systemImage: ${kind.checked} ? \"checkmark.square.fill\" : \"square\") }"}" +
                    enabled(node) + accessibility(node)
...
private object ComposeGenerator : CodeGenerator {
    ...
    override fun generate(document: ResolvedDocument, typeName: String): GeneratedSource {
        val body = composeNode(document.document.root, 1)
        return GeneratedSource(
            target,
            target.fileName,
            """
            ...
            |@Composable
            |fun $typeName(dispatch: (action: String, value: String?) -> Unit) {
            |$body
            |}
            |""".trimMargin(),
        )
    ...
    private fun composeNode(node: Node, depth: Int): String {
        val i = "    ".repeat(depth)
        val child = { extra: Int ->
            node.children.joinToString("\n") { composeNode(it, depth + extra) }
        }
        return when (val kind = node.kind) {
            ...
            is NodeKind.Input -> {
                ...
                "$i${"OutlinedTextField(value = \"${kind.value.kotlin()}\", onValueChange = { dispatch(\"${kind.onChange.kotlin()}\", it) }, label = { Text(\"${kind.placeholder.orEmpty().kotlin()}\") }, enabled = ${node.semantics.enabled}, keyboardOptions = $keyboard"}" +
            ...
            is NodeKind.Navigation -> {
                ...
                if (kind.mode == NavigationMode.Tab) {
                    "$i${"Column {\n${"    ".repeat(depth + 1)}NavigationBar {\n"}" +
                        ...
                        selected?.let { composeNode(it, depth + 1) }.orEmpty() + "\n$i}"
                } else {
                    selected?.let { composeNode(it, depth) } ?: "${i}Spacer(Modifier)"
                }
            }
            is NodeKind.Route -> "$i${"Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {\n${child(1)}\n$i}"}"
            is NodeKind.PlatformView ->
                throw IllegalArgumentException(
                    "Platform view '${kind.name}' needs an explicit Compose escape hatch.",
                )
            is NodeKind.Toggle ->
                "$i${"Row { Switch(checked = ${kind.checked}, onCheckedChange = { dispatch(\"${kind.onChange.kotlin()}\", it.toString()) }, enabled = ${node.semantics.enabled})"}" +
            ...
            is NodeKind.Slider ->
                "$i${"Slider(value = ${kind.value}f, onValueChange = { dispatch(\"${kind.onChange.kotlin()}\", it.toString()) }, valueRange = ${kind.min}f..${kind.max}f, enabled = ${node.semantics.enabled})"}"
            is NodeKind.Picker ->
                "$i${"Column { Text(\"${kind.options.firstOrNull { it.value == kind.selected }?.label.orEmpty().kotlin()}\")"}" +
                    kind.options.joinToString("") {
            ...
            is NodeKind.Checkbox ->
                "$i${"Row { Checkbox(checked = ${kind.checked}, onCheckedChange = { dispatch(\"${kind.onChange.kotlin()}\", it.toString()) }, enabled = ${node.semantics.enabled})"}" +
                    (kind.label?.let { "; Text(\"${it.kotlin()}\")" } ?: "") + " }"
...
private object WinUiGenerator : CodeGenerator {
    ...
    override fun generate(document: ResolvedDocument, typeName: String): GeneratedSource {
        val body = xamlNode(document.document.root, 2)
        return GeneratedSource(
            target,
            target.fileName,
            """
    ...
    private fun xamlNode(node: Node, depth: Int): String {
        val i = "    ".repeat(depth)
        val child = { extra: Int ->
            node.children.joinToString("\n") { xamlNode(it, depth + extra) }
        }
        ...
        return when (val kind = node.kind) {
            ...
            is NodeKind.Input -> {
                ...
                "$i<$control $value=\"${kind.value.xml()}\" PlaceholderText=\"${kind.placeholder.orEmpty().xml()}\" Tag=\"${kind.onChange.xml()}\"$enabled$automation />"
            }
            is NodeKind.Stack -> {
            ...
            is NodeKind.Navigation -> {
                ...
                "$i<NavigationView PaneDisplayMode=\"${if (kind.mode == NavigationMode.Tab) "Top" else "Left"}\">\n" +
                    ...
                    selected?.let { xamlNode(it, depth + 2) }.orEmpty() +
                    "\n${"    ".repeat(depth + 1)}</NavigationView.Content>\n$i</NavigationView>"
            }
            is NodeKind.Route -> "$i<StackPanel Spacing=\"16\">\n${child(1)}\n$i</StackPanel>"
            is NodeKind.PlatformView ->
                throw IllegalArgumentException(
                    "Platform view '${kind.name}' needs an explicit WinUI escape hatch.",
                )
            is NodeKind.Toggle ->
                "$i<ToggleSwitch Header=\"${kind.label.orEmpty().xml()}\" IsOn=\"${kind.checked}\" Tag=\"${kind.onChange.xml()}\"$enabled$automation />"
            ...
            is NodeKind.Slider ->
                "$i<Slider Value=\"${kind.value}\" Minimum=\"${kind.min}\" Maximum=\"${kind.max}\" StepFrequency=\"${kind.step ?: 1.0}\" Tag=\"${kind.onChange.xml()}\"$enabled$automation />"
            is NodeKind.Picker ->
                "$i<ComboBox SelectedValue=\"${kind.selected.xml()}\" Tag=\"${kind.onChange.xml()}\"$enabled>\n" +
                    kind.options.joinToString("\n") {
            ...
            is NodeKind.Checkbox ->
                "$i<CheckBox Content=\"${kind.label.orEmpty().xml()}\" IsChecked=\"${kind.checked}\" Tag=\"${kind.onChange.xml()}\"$enabled$automation />"
            NodeKind.Divider ->
