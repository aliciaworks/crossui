...
On macOS hosts the typed boundary is generated by Kotlin **Swift Export**
(`swiftExport` in `runtime/build.gradle.kts` and the feature build script):
Kotlin state classes become `Sendable` Swift structs, sealed actions become
Swift enums with associated values, `StateFlow` becomes a typed Swift sequence,
and `@MainActor` isolation is preserved. The three bridges above are therefore
thin adapters over real Swift value types rather than ObjC interop shims, and
the connected view dispatches pattern-matchable enum actions.
